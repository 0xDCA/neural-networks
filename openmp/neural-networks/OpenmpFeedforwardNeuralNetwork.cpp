...
OpenmpFeedforwardNeuralNetwork::OpenmpFeedforwardNeuralNetwork(const std::vector<int>& layers) : layers(layers) {
    if (layers.size() < 2) {
...
void OpenmpFeedforwardNeuralNetwork::back_propagation(const std::vector<Eigen::VectorXd> &fp_results,
...
std::vector<Eigen::VectorXd> OpenmpFeedforwardNeuralNetwork::forward_propagation(const Eigen::VectorXd &input,
                                                                           const std::vector<Eigen::MatrixXd> &weight_list) {
...
std::vector<Eigen::VectorXd> OpenmpFeedforwardNeuralNetwork::forward_propagation(const Eigen::VectorXd &input) const {
    return forward_propagation(input, this->weight_list);
}

void OpenmpFeedforwardNeuralNetwork::set_weights_eigen(int source_layer, const Eigen::MatrixXd &weights) {
...
Eigen::MatrixXd OpenmpFeedforwardNeuralNetwork::get_weights_eigen(int source_layer) const {
    if (source_layer < 0 || source_layer >= static_cast<int>(weight_list.size())) {
        throw runtime_error("Invalid layer index");
    }
...
Eigen::VectorXd OpenmpFeedforwardNeuralNetwork::predict(const Eigen::VectorXd &input) const {
    std::vector<Eigen::VectorXd> all_results = forward_propagation(input);

    return all_results[all_results.size() - 1];
}

Eigen::MatrixXd OpenmpFeedforwardNeuralNetwork::random_matrix(int rows, int cols, double epsilon, std::mt19937& generator) {
...
TrainResult OpenmpFeedforwardNeuralNetwork::train(const Eigen::MatrixXd &x, const Eigen::MatrixXd &y,
    ...
    if (train_settings.initialize_weights_randomly) {
      for(size_t i = 0; i < weight_list.size(); ++i) {
          set_weights_eigen(i, OpenmpFeedforwardNeuralNetwork::random_matrix(
...
void OpenmpFeedforwardNeuralNetwork::do_gradient_descent(const WorkerParams& params) {
...
double OpenmpFeedforwardNeuralNetwork::compute_error(const Eigen::MatrixXd& x, const Eigen::MatrixXd& y, double regularization_term,
  const std::vector<Eigen::MatrixXd>& weight_list) {
    if (x.rows() != y.rows()) {
...
double OpenmpFeedforwardNeuralNetwork::compute_error(const Eigen::MatrixXd &x, const Eigen::MatrixXd &y,
                                               double regularization_term) const {
    return compute_error(x, y, regularization_term, this->weight_list);
}

std::vector<Eigen::MatrixXd> OpenmpFeedforwardNeuralNetwork::compute_weights_error(const Eigen::VectorXd &x,
...
std::vector<std::vector<double> > OpenmpFeedforwardNeuralNetwork::get_activations(const std::vector<double>& input) const {
...
std::vector<double> OpenmpFeedforwardNeuralNetwork::predict(const std::vector<double>& input) const {
  return eigenVectorToStdVector(predict(stdVectorToEigenVector(input)));
}

std::vector<dc::ManagedMatrix<double> > OpenmpFeedforwardNeuralNetwork::compute_weights_error(const std::vector<double>& input,
...
TrainResult OpenmpFeedforwardNeuralNetwork::train(const dc::ManagedMatrix<double>& x, const dc::ManagedMatrix<double>& y,
    const TrainSettings& train_settings) {
  return train(managedMatrixToEigenMatrix(x).transpose(), managedMatrixToEigenMatrix(y).transpose(), train_settings);
}

void OpenmpFeedforwardNeuralNetwork::set_weights(int source_layer, const dc::ManagedMatrix<double> &weights) {
  set_weights_eigen(source_layer, managedMatrixToEigenMatrix(weights));
}

dc::ManagedMatrix<double> OpenmpFeedforwardNeuralNetwork::get_weights(int source_layer) const {
  return eigenMatrixToManagedMatrix(get_weights_eigen(source_layer));
}

double OpenmpFeedforwardNeuralNetwork::compute_error(const dc::ManagedMatrix<double>& x, const dc::ManagedMatrix<double>& y,
