...
OpenclFeedforwardNeuralNetwork::OpenclFeedforwardNeuralNetwork(const std::vector<int>& layer_neurons) :
...
std::vector<std::vector<double> > OpenclFeedforwardNeuralNetwork::get_activations(const std::vector<double>& input) const {
	return opencl_helper.forward_propagate(layer_neurons, input, weights);
}

std::vector<double> OpenclFeedforwardNeuralNetwork::predict(const std::vector<double>& input) const {
	return get_activations(input).back();
}

std::vector<dc::ManagedMatrix<double> > OpenclFeedforwardNeuralNetwork::compute_weights_error(
		const std::vector<double>& input, const std::vector<double>& output) const {
	return opencl_helper.back_propagate(layer_neurons, input, output, weights);
}

TrainResult OpenclFeedforwardNeuralNetwork::train(const dc::ManagedMatrix<double>& x,
	...
	auto result = opencl_helper.train(layer_neurons, x, y, train_settings, weights);
...
dc::ManagedMatrix<double> OpenclFeedforwardNeuralNetwork::get_weights(int source_layer) const {
...
void OpenclFeedforwardNeuralNetwork::set_weights(int source_layer, const dc::ManagedMatrix<double> &weights) {
	if (source_layer < 0 || source_layer > layer_neurons.size()) {
		throw std::runtime_error("Invalid layer number");
	}
...
void OpenclFeedforwardNeuralNetwork::fill_matrix_randomly(dc::ManagedMatrix<double>& matrix,
...
double OpenclFeedforwardNeuralNetwork::compute_error(const dc::ManagedMatrix<double>& x,
...
double OpenclFeedforwardNeuralNetwork::compute_error_gpu(const dc::ManagedMatrix<double>& x,
	...
	return opencl_helper.compute_error(layer_neurons, x, y, weights, regularization_term,
